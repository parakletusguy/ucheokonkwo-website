  ...
  return (
    <div className="mb-10">
      ...
      >
        ...
        <img src={images[selected].url} alt={`Image ${selected + 1}`} className="w-full h-full object-cover" />
        {images.length > 1 && (
          <>
            <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 hover:bg-black/70 text-white flex items-center justify-center transition-all">
              <span className="material-symbols-outlined text-lg">chevron_left</span>
            </button>
            <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 hover:bg-black/70 text-white flex items-center justify-center transition-all">
      ...
      {images.length > 1 && (
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {images.map((img, i) => (
            <button key={img.id} onClick={() => setSelected(i)} className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${i === selected ? 'border-[var(--midnight-green)] opacity-100' : 'border-transparent opacity-50 hover:opacity-80'}`}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={() => setLightbox(false)}>
          ...
          {images.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center">
                <span className="material-symbols-outlined">chevron_left</span>
              </button>
              <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center">
                <span className="material-symbols-outlined">chevron_right</span>
              </button>
            </>
          )}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={images[selected].url} alt="" className="max-h-[90vh] max-w-[90vw] object-contain" onClick={(e) => e.stopPropagation()} />
          <span className="absolute bottom-4 text-white/50 text-sm">{selected + 1} / {images.length} — press ← → to navigate, Esc to close</span>
        </div>
      )}
    </div>
  );
...
/* ── Detail Client ────────────────────────────────────────────────────────── */
export default function PostDetailClient({ id, initialPost }: { id: string; initialPost: Post | null }) {
  const [post, setPost]             = useState<Post | null>(initialPost);
  const [loading, setLoading]       = useState(!initialPost);
  const [error, setError]           = useState('');
  const [activeLang, setActiveLang] = useState(() => {
    const langs = initialPost?.multilingualContent?.map(m => m.language) ?? [];
    return langs.includes('ENGLISH') ? 'ENGLISH' : (langs[0] ?? 'ENGLISH');
  });

  // Only fetch if we didn't get server-side data
  useEffect(() => {
    ...
    fetch(`${API_BASE}/posts/${id}`)
      .then(res => {
        if (res.status === 404) { if (!cancelled) setError('not_found'); return null; }
  ...
  if (error === 'not_found' || !post) return (
    <>
      ...
      <main className="pt-32 pb-24 max-w-3xl mx-auto px-6 flex flex-col items-center justify-center min-h-[60vh] gap-4 text-gray-400">
        ...
        <Link href="/media" className="mt-4 bg-[var(--midnight-green)] text-[var(--sunlight-yellow)] px-6 py-3 text-xs font-bold uppercase tracking-widest rounded-sm">Back to Media Hub</Link>
      </main>
      <Footer />
    </>
  );
  ...
  return (
    <>
      ...
      <main className="pt-28 pb-24 max-w-3xl mx-auto px-6">
        ...
        <h1 className="text-3xl md:text-5xl font-bold text-[var(--obsidian)] serif-font leading-tight mb-4">{displayTitle}</h1>
        ...
        {mc.length > 1 && (
          <div className="flex gap-2 flex-wrap mb-8">
            {mc.map(m => (
              <button key={m.language} onClick={() => setActiveLang(m.language)}
                className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${activeLang === m.language ? 'bg-[var(--midnight-green)] text-[var(--sunlight-yellow)]' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
