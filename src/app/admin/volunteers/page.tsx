            ...
            {filtered.map((v) => (
              <div key={v.id} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-[var(--midnight-green)]/10 flex items-center justify-center flex-shrink-0">
                  <span className="text-[var(--midnight-green)] font-bold text-sm">{v.name.charAt(0).toUpperCase()}</span>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--midnight-green)] mt-0.5">{v.ward}</p>
                </div>
