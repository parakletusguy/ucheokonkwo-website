        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center w-full h-full transition-colors ${isActive ? 'text-[var(--sunlight-yellow)]' : 'text-blue-200 hover:text-white'}`}
            >
              <span className={`material-symbols-outlined text-2xl mb-1 ${isActive ? 'text-[var(--sunlight-yellow)]' : ''}`}>
                {item.icon}
              </span>
